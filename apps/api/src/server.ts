...
import { getQueue, getRedis, type NfeJobPayload } from "./queue.js";
...
function jobDir(id: string): string {
  return path.join(env.TEMP_JOBS_ROOT, id);
}
...
app.get<{ Params: { id: string } }>(`${API_PREFIX}/jobs/:id`, async (req, reply) => {
  ...
  if (status === "done") {
    ...
    downloadToken = await signDownloadToken(env, id, fileName);
  }
  if (status === "failed") {
